...
from experiments.configurations import INSTANCE_SIZES, REPETITIONS
...
def build_parser() -> argparse.ArgumentParser:
    ...
    experiment.add_argument("--initial-capacity", type=int, default=16)
    experiment.add_argument("--overwrite", action="store_true")

    analyze = subparsers.add_parser("analyze", help="summarize raw results")
    analyze.add_argument("--input", type=Path, default=Path("results/raw/results.csv"))
    analyze.add_argument("--output", type=Path, default=Path("results/summary/summary.csv"))

    plot = subparsers.add_parser("plot", help="generate all figures")
    plot.add_argument("--input", type=Path, default=Path("results/raw/results.csv"))
    plot.add_argument("--figures-dir", type=Path, default=Path("figures"))
...
def main() -> None:
    ...
    if args.command == "experiment":
        ...
        print("\nCompleted.\nResults saved to results/raw/results.csv")
    elif args.command == "analyze":
        summarize_results(args.input, args.output)
        print(f"Summary saved to {args.output}")
    elif args.command == "plot":
        created = generate_plots(args.input, args.figures_dir)
        ...
        print("Results saved to results/raw/threshold_experiment.csv")
        ...
        print("Results saved to results/raw/collision_experiment.csv")
